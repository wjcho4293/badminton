...
app.post('/auth/signup', async (req, res) => {
  ...
  if (!username || !password || !name) {
    return res.redirect('/?view=auth&authTab=signup&toast=' + encodeURIComponent('필수 정보를 모두 입력해주세요.'));
  }
  ...
  try {
    ...
    db.run(`INSERT INTO users (username, password, name, age, carrot_nickname, gender, role) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [username, hashedPassword, name, age, carrot_nickname, gender, role], (err) => {
        if (err) {
          return res.redirect('/?view=auth&authTab=signup&toast=' + encodeURIComponent('이미 존재하는 아이디이거나 회원가입 실패입니다.'));
        }
        db.get(`SELECT * FROM users WHERE username = ?`, [username], (err, user) => {
          req.session.user = user;
          res.redirect('/?tab=profile&toast=' + encodeURIComponent('회원가입 및 자동 로그인 완료!'));
        });
      }
    );
  } catch (error) {
    res.redirect('/?view=auth&authTab=signup&toast=' + encodeURIComponent('회원가입 처리 중 오류가 발생했습니다.'));
  }
});
...
app.post('/auth/login', (req, res) => {
  ...
  if (!username || !password) {
    return res.redirect('/?view=auth&authTab=login&toast=' + encodeURIComponent('아이디와 비밀번호를 입력해주세요.'));
  }

  db.get(`SELECT * FROM users WHERE username = ?`, [username], async (err, user) => {
    if (err || !user) {
      return res.redirect('/?view=auth&authTab=login&toast=' + encodeURIComponent('아이디 또는 비밀번호가 일치하지 않습니다.'));
    }
    ...
    req.session.user = user;
    res.redirect('/?tab=profile&toast=' + encodeURIComponent(`${user.name}님 환영합니다!`));
  });
});
...
app.post('/meetups/:meetupId/join', (req, res) => {
  ...
  if (!currentUser) {
    return res.redirect('/?view=auth&toast=' + encodeURIComponent('로그인이 필요합니다.'));
  }
...
app.post('/global-chats', (req, res) => {
  const currentUser = req.session.user;
  if (!currentUser) {
    return res.redirect('/?tab=global_chat&toast=' + encodeURIComponent('로그인이 필요합니다.'));
  }
  const { message } = req.body;
  if (!message || !message.trim()) {
...
app.post('/meetups/create', (req, res) => {
  const currentUser = req.session.user;
  if (!currentUser) {
    return res.redirect('/?view=auth&toast=' + encodeURIComponent('모임을 개설하려면 로그인이 필요합니다.'));
  }
