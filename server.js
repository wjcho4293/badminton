      ...
      const injectedScript = `
        <script>
          document.addEventListener("DOMContentLoaded", () => {
            ...
            // 기존 중복 뱃지 제거 후 재생성 (중복 표시 방지 및 크기/줄바꿈 방지 적용)
            document.querySelectorAll(".duo-badge, .court-duo-badge").forEach(el => el.remove());

            // 출석부/듀오 설정 체크박스 라벨에 뱃지 추가 (줄 바꿈 방지 스타일 적용)
            ...
            // 1. 듀오 지정 박스 내의 인원은 항상 모두 표시 (재지정 및 해제 가능)
            if (duoForm) {
            ...
            // 2. 일반적인 출석 인원은 코트에 들어가면 자동으로 숨기고, 경기가 종료되면 다시 표시
            ...
            // 코트에 들어간 선수 이름 왼쪽에 듀오 뱃지 추가 (줄 바꿈 방지 및 크기 고정)
            setTimeout(() => {
          ...
          // 듀오 체크박스 변경 시 2명이 선택된 경우에만 자동 제출
...
const db = new sqlite3.Database(':memory:', (err) => {
  ...
  else console.log('SQLite 인메모리 DB 연결 성공');
});
        ...
        [court.teamA_p1, court.teamA_p2, court.teamB_p1, court.teamB_p2, scoreA, scoreB, winner], function() {
          ...
          db.all(`SELECT * FROM users WHERE name IN (?, ?, ?, ?)`, [court.teamA_p1, court.teamA_p2, court.teamB_p1, court.teamB_p2], (err, users) => {
            if (users && users.length > 0) {
              ...
              const scoreDiffFactor = 1 + Math.abs(scoreA - scoreB) / 20;

              const changeAmount = Math.round(kFactor * scoreDiffFactor * ( (winner === 'A' ? 1 : 0) - expectedA ));
